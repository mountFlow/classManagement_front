    ...
    <div style="margin: 20px; height: 99%">
      ...
      >
        ...
        <el-form-item label="课室名称" style="margin-left:20px">
          <el-select v-model="classroomAvailableSearch.classroomId" placeholder=""  size="small" style="width:80px" >
            <el-option
              v-for="item in classroomNames"
              :key="item[0]"
              :label="item[1]"
        ...
        <el-form-item label="周数">
          <el-select v-model="classroomAvailableSearch.week" placeholder=""  size="small" style="width:100px" >
            <el-option
              v-for="item in optionsForWeek"
               :key="item.value"
              :label="item.label"
        ...
        <el-form-item label="星期" >
          <el-select v-model="classroomAvailableSearch.date" placeholder=""  size="small" style="width:100px">
          <el-option
              v-for="item in optionsForDate"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item label="节数">
          <el-select v-model="classroomAvailableSearch.time" placeholder="" size="small" style="width:100px" >
            <el-option
              v-for="item in optionsForTime"
              :key="item.value"
              :label="item.label"
...
下面开始是tabledata内容 -->
      ...
      <el-table
        ...
        height="540"
      >
        <el-table-column prop="classroomId" align="center" label="可用课室" width="200" sortable>
        </el-table-column>
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button
              ...
              @click="handleEdit(scope.row),add(),selectAll()"
              >申请</el-button
            >
          </template>
        </el-table-column>
...
} from "@/utils/classroomAvailableapi";
import {
  addApi,
...
export default {
  ...
  methods: {
    ...
    add() {
      ...
      var temp=this.DataTranslateRev(this.classroomAvailableTemp)
      temp.status="等待审核"
      console.log("------333333333------------------");
console.log(temp);

console.log("------333333333------------------");
      addApi(temp).then((resp) => {
    ...
    update() {
      this.$confirm("确定修改?", "提示", {
        ...
        updateApi(this.classroomAvailable).then((resp) => {
          // console.log(resp);
          if (resp == "success") {
            //5.添加成功则关闭对话框，重新查询数据,弹出提示添加成功消息框
            this.dialogVisible = false;
    ...
    //编辑某一行的回调函数
    handleEdit(row) {
      this.classroomAvailableTemp = row;
    },
    giveBasicValue() {
      this.classroomAvailable.id = "";
      this.classroomAvailable.classroomId = "";
      this.classroomAvailable.week = "第一周";
      this.classroomAvailable.time = "① 8:00-8:40";
      this.classroomAvailable.date = "星期一";
      this.classroomAvailableSearch.id = "";
      this.classroomAvailableSearch.classroomId = "";
      this.classroomAvailableSearch.week = "";
      this.classroomAvailableSearch.time = "";
      this.classroomAvailableSearch.date = "";
    },
    onSearch() {
      console.log(this.classroomAvailableSearch);
      selectByConditionApi(this.classroomAvailableSearch).then((resp) => {
